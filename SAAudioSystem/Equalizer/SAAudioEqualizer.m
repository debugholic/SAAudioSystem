...
@implementation SAAudioEqualizer
...
- (AVFilterGraph *)drawFilter {
    ...
    uint64_t ch_layout = av_get_default_channel_layout((int)_metadata.channels);
    av_get_channel_layout_string(ch_layout_str, sizeof(ch_layout_str), 0, ch_layout);
...
- (int)adjust:(void *)data length:(size_t)length flag:(SAAudioEqualizerFlag)flag {
    ...
    src->channel_layout = av_get_default_channel_layout((int)_metadata.channels);
    ...
    @synchronized (self) {
        ...
        if (flag == SAAudioEqualizerFlagNone) {
            if (_metadata.bitdepth == 24) {
            ...
            for (int i = 0; i < nb_samples; i += channels) {
                ...
                for (int j = 0; j < channels; j++) {
                    if (_metadata.bitdepth == 16) {
                        if (flag == SAAudioEqualizerFlagOn) {

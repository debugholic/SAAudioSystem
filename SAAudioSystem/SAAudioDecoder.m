...
@property (assign, nonatomic) SAPacketQueue packetQueue;
@property (assign, nonatomic) SAPacketQueue cacheQueue;
...
NSString * const SAAudioDecoderErrorDomain = @"com.sidekick.academy.error.audio.decoder";

@implementation SAAudioDecoder
...
- (void)openSource:(NSString *)sourcePath error:(NSError *__autoreleasing *)error {
    if (!sourcePath) {
    ...
    if (ret < 0) {
        if (AVERROR(ret) == AVERROR_HTTP_SERVER_ERROR) {
            *error = [NSError errorWithDomain:SAAudioDecoderErrorDomain
                                         code:SAAudioSystemErrorServerError
                                     userInfo:@{NSLocalizedDescriptionKey:@"Could not connect a server"}];
        } else {
            *error = [NSError errorWithDomain:SAAudioDecoderErrorDomain
                                         code:SAAudioSystemErrorNotOpenFile
    ...
    if (_audioStreamIndex == -1) {
        *error = [NSError errorWithDomain:SAAudioDecoderErrorDomain
                                     code:SAAudioSystemErrorNotFoundAudioStream
...
- (int)decodeFrameInAQBufferCapacity:(UInt32)bufferCapacity outAQBuffer:(UInt8 *)buffer error:(NSError **)error {
    ...
    // Planar and double type data.
        ...
        switch (_codecContext->sample_fmt) {
            case AV_SAMPLE_FMT_U8P :
                av_samples_alloc(&cvtData, NULL, _codecContext->channels, _frame->nb_samples, AV_SAMPLE_FMT_U8, 0);
                break;
                
            case AV_SAMPLE_FMT_S16P :
                av_samples_alloc(&cvtData, NULL, _codecContext->channels, _frame->nb_samples, AV_SAMPLE_FMT_S16, 0);
                break;
                
            case AV_SAMPLE_FMT_S32P :
                av_samples_alloc(&cvtData, NULL, _codecContext->channels, _frame->nb_samples, AV_SAMPLE_FMT_S32, 0);
                break;
                
            case AV_SAMPLE_FMT_FLTP :
            case AV_SAMPLE_FMT_DBL :
            case AV_SAMPLE_FMT_DBLP :
                av_samples_alloc(&cvtData, NULL, _codecContext->channels, _frame->nb_samples, AV_SAMPLE_FMT_FLT, 0);
                break;
...
- (void)seekFrameToPos:(int64_t)pos error:(NSError **)error {
    ...
    if (!_formatContext) {
        *error = [NSError errorWithDomain:SAAudioDecoderErrorDomain
                                     code:SAAudioSystemErrorSeekingFailed
